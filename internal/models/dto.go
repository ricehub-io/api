...
func getUserAvatar(avatarPath *string) string {
	cdnURL := config.Config.App.CDNUrl
	avatar := cdnURL + config.Config.App.DefaultAvatar
	if avatarPath != nil {
...
func (df RiceDotfiles) ToDTO() RiceDotfilesDTO {
	dto := RiceDotfilesDTO{
		FilePath:  config.Config.App.CDNUrl + df.FilePath,
...
func (p RiceScreenshot) ToDTO() RiceScreenshotDTO {
	return RiceScreenshotDTO{
		ID:  p.ID,
		Url: config.Config.App.CDNUrl + p.FilePath,
	}
}
...
func (r PartialRice) ToDTO() PartialRiceDTO {
	return PartialRiceDTO{
		...
		Username:    r.Username,
		Thumbnail:   config.Config.App.CDNUrl + r.Thumbnail,
		Stars:       r.StarCount,
